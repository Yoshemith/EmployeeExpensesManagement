...
public class EmployeesDatabaseImpl implements Employees{
    ...
    private List<ExpenseItem> getExpenseItemsForClaims(int claimId) {
        ...
        try (Connection connection = DriverManager.getConnection("jdbc:h2:./expenses", "sa", "")) {
            ...
            while (rs.next()) {
                ...
                ExpenseItem ei  = new ExpenseItem(id, claimId, ExpenseType.valueOf(expenseType.toUpperCase()), description, amount);
                itemsList.add(ei);
    ...
    private List<ExpenseClaim> getExpenseClaimsForEmployee(int employeeId) {
        ...
        try (Connection connection = DriverManager.getConnection("jdbc:h2:./expenses", "sa", "")) {
            ...
            while (rs.next()) {
                ...
                List<ExpenseItem> expenseItems = getExpenseItemsForClaims(id);
    ...
    private void addExpenseItem(ExpenseItem item) {
        try (Connection connection = DriverManager.getConnection("jdbc:h2:./expenses", "sa", "")) {
            Statement statement = connection.createStatement();
            String sql = "INSERT INTO expenseitems (id, claimId, expenseType, description, amount) " +
                    "VALUES ( " + item.getId() + ", " + item.getClaimId() + ", '" + item.getExpenseType().toString() +"', '" +
                    item.getDescription() + "', " +
                    item.getAmount() + ")";
            statement.executeUpdate(sql);
    ...
    public void addExpenseClaim(ExpenseClaim claim) throws EmployeeNotFoundException {
        try (Connection connection = DriverManager.getConnection("jdbc:h2:./expenses", "sa", "")) {
            Statement statement = connection.createStatement();
            String sql = "INSERT INTO expenseclaims (id, employeeId, dateOfClaim, approved, paid) " +
